...
var defaults = {
    ...
    child: {
        ...
        'cookies-file': '/tmp/spookycookies.txt'
    },
    casper: {
        verbose: true,
        logLevel: 'debug'
...
function Spooky(options, callback, cookiesFile) {
    this.options = options = _.defaults(_.clone(options || {}), defaults);

    if (typeof cookiesFile !== 'undefined') {
        options.child['cookies-file'] = this.options.child['cookies-file'] = cookiesFile;
    }

    console.log(this.options.child);
